...
class OSSUploader {
    ...
    private init() {
        guard
            ...
            let keySecret = dict["OSSAccessKeySecret"]
        else {
    ...
    /// 上传文件到 OSS，返回填好 fullURL 的 WallpaperItem
    func uploadFile(fileURL: URL, fileData: Data, draft: WallpaperItem) async throws -> WallpaperItem {
    ...
    private func putObject(objectKey: String, data: Data, fileExtension: String) async throws {
        ...
        let signature = HMAC<Insecure.SHA1>.authenticationCode(
            for: stringToSign.data(using: .utf8)!, using: key
        )
